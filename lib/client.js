...
function buildStageDetail(mapKey, stageKey, status, summary) {
    ...
    const parts = [];
    if (status === 'active')
        parts.push(`当前正在「${label}」。`);
    else if (status === 'done')
        parts.push(`「${label}」已经完成。`);
    else
        parts.push(`「${label}」还没有开始。`);
    ...
    if (phase?.count) {
        ...
        if (phase.files?.length)
            parts.push(`主要落在：${phase.files.slice(0, 4).join('、')}。`);
    }
...
function AiBlackboxFlow(props) {
    ...
    return createElement('div', { className: 'ai-blackbox-flow' }, [
        ...
        ArchifyWorkMap({ summary, status, selectedKey: detailStage, onSelect: (key) => setDetailStage(key === detailStage ? null : key) }),
