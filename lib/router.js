...
import { demotedSkillIds, loadEffectMetrics } from './effects.js';
export const DOMAIN_SCENARIOS = ['teaching', 'learning', 'research', 'github', 'dsh-ops', 'writing'];
...
export class SkillRouterManager {
    ...
    cache = null;
    catalogPromise = null;
    catalogPromiseGeneration = -1;
    catalogGeneration = 0;
    catalogLoadedAt = 0;
    constructor(vault, getSessionId, effectProvider) {
        this.vault = vault;
    ...
    async applyDecision(decision, sessionId) {
        ...
        for (const id of demoted) {
            ...
            if (!row)
                continue;
            if (row.routing === 'base' || row.activation === 'always-on')
                continue;
            enable.delete(id);
            disable.add(id);
        }
    ...
    async getCatalog() {
        ...
        if (this.catalogPromise && this.catalogPromiseGeneration === this.catalogGeneration)
            return this.catalogPromise;
        // 短 TTL 缓存：避免高频 status/suggested 反复请求 vault 目录。
        const now = Date.now();
        if (this.cache && now - this.catalogLoadedAt < CATALOG_TTL_MS)
            return this.cache;
    ...
    setRoute(sid, decision) {
        this.routes.set(sid, { decision, at: Date.now() });
    }
    suggestedFor(route, scenario, catalog, demoted = []) {
        const demotedSet = new Set(demoted);
        const notDemoted = (e) => !demotedSet.has(e.id);
        if (route === 'core') {
