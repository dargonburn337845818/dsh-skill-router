...
export function loadEffectMetrics(file = effectLogPath()): EffectMetricsRow[] {
  ...
  const map = new Map<string, EffectMetricsRow>()
  const latest = new Map<string, string>()
  for (const e of entries) {
    if (!e || typeof e.skill !== 'string') continue
    ...
    if (e.at && (!latest.get(e.skill) || e.at > latest.get(e.skill)!)) latest.set(e.skill, e.at)
  }
...
export function demotedSkillIds(rows: EffectMetricsRow[], opts: DemoteOptions = {}): Set<string> {
  const minSamples = opts.minSamples ?? 3
  const effectThreshold = opts.effectThreshold ?? 0.4
  const misuseThreshold = opts.misuseThreshold ?? 0.3
  const out = new Set<string>()
  for (const r of rows) {
    if (r.total < minSamples) continue
    if (r.effectRate < effectThreshold || r.misuseRate > misuseThreshold) out.add(r.skill)
  }
  return out
}

/** Skills with strong positive evidence; can be promoted into suggestions. */
export function promotedSkillIds(rows: EffectMetricsRow[], opts: DemoteOptions = {}): Set<string> {
  const minSamples = opts.minSamples ?? 3
  const out = new Set<string>()
  for (const r of rows) {
    if (r.total < minSamples) continue
    if (r.effectRate >= 0.5 && r.misuseRate <= 0.3) out.add(r.skill)
  }
  return out
}
