...
export class SkillRouterManager {
  ...
  async switch(route: RouterRoute, scenario?: string, sessionId?: string): Promise<RouteDecision> {
    const sid = sessionId || this.getSessionId()
    const decision: RouteDecision = { route, scenario, confidence: 'high' }
    await this.applyDecision(decision, sid)
    return decision
  }
  ...
  private async applyDecision(decision: RouteDecision, sessionId?: string): Promise<void> {
    ...
    const enable = new Set<string>(baseIds)
    const disable = new Set<string>(coreIds)

    for (const sc of DOMAIN_SCENARIOS) disable.add(sc)
  ...
  async resetBase(): Promise<{ ok: boolean; baseIds: string[] }> {
    const catalog = await this.getCatalog()
    const baseIds = catalog.entries.filter((e) => e.routing === 'base' || e.activation === 'always-on').map((e) => e.id)
