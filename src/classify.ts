...
const CORE_RE =
  /蒸馏|提炼|做成skill|做成技能|知识节点|迭代|收敛|元能力|收益|filter|distill|iterate|validate|收敛判断|价值驱动/i

const DEV_RE =
  /开发|编码|写代码|重构|修复|调试|实现|插件|项目|架构|模块|接口|部署|build|develop|implement|refactor|fix|debug|code|plugin/i

const SEARCH_RE =
  /查资料|搜索|调研|检索|来源|github|论文|学术|包生态|npm|pypi|osv|漏洞|release|commit|文档|查一下|找一下|search|research|source/i
...
export function classifyRoute(text: string): RouteDecision {
  const t = String(text || '')
  if (!t.trim()) return { route: 'base', confidence: 'low' }

  if (CORE_RE.test(t)) return { route: 'core', confidence: 'high' }
  ...
  for (const domain of DOMAIN_RE) {
    if (domain.re.test(t)) return { route: 'domain', scenario: domain.scenario, confidence: 'high' }
  }

  if (DEV_RE.test(t)) return { route: 'dev', confidence: 'high' }

  if (SEARCH_RE.test(t)) return { route: 'base', confidence: 'medium' }

  // 无法判别时回到常驻搜索底座，让用户在面板显式切换。
  return { route: 'base', confidence: 'low' }
}

export function routeLabel(route: string, scenario?: string): string {
  if (route === 'core') return '核心 Skill 迭代'
  if (route === 'dev') return '开发 / 重构'
  if (route === 'domain') {
