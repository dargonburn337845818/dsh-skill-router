...
/**
 * AI 黑盒趋势 —— 把当前会话的“用户输入 / AI 阶段 / 工具调用 / 命令 / 异常”实时画成一张流程图。
 * 数据源：Trajectory 插件提供的 `useTrajectory`（会话事件 + 运行中工具） + `/skill-router/api/status`（当前工作流）。
 * 不做静态 HTML，不生成文件，只在这个视图里实时刷新。
 */
type FlowKind = 'prompt' | 'think' | 'tool' | 'command' | 'error' | 'compaction'
...
function domainOf(label: string, detail: string): string {
  const s = `${label} ${detail}`.toLowerCase()
  if (/expert|teacher|教学|讨论|专家团/.test(s)) return 'teacher'
  if (/arxiv|doi|论文|科研|research|web_search|web_fetch|search|source/.test(s)) return 'research'
  if (/prompt|文案|文稿|writing|report|ppt|copywriting/.test(s)) return 'writing'
  if (/vault|skill|蒸馏|distill|core-iteration|元能力|迭代/.test(s)) return 'distill'
  if (/bash|git|build|dev_|grep|glob|edit|tsc|code|plugin|fix|debug|npm|python/.test(s)) return 'dev'
  return 'base'
}

function buildFlowNodes(trajectory: any): FlowNode[] {
  ...
  for (const n of trajectory.eventNodes ?? []) {
    if (n.kind === 'user') {
      ...
      const reason = (n.blocks ?? []).filter((b: any) => b.kind === 'reasoning').map((b: any) => b.text).join(' ')
      const text = (n.blocks ?? []).filter((b: any) => b.kind === 'text').map((b: any) => b.text).join(' ')
      if (reason || text) {
...
function kindText(kind: FlowKind): string {
  switch (kind) {
...
function FlowGraph({ nodes, onOpen }: { nodes: readonly FlowNode[]; onOpen?: (callId: string) => void }): any {
  const cols = 4
  const nodeW = 150
  const nodeH = 54
  const gapX = 86
...
const PHASE_META: Record<string, { title: string; icon: string }> = {
...
function toolArgsRaw(node: any): string {
...
function toolPhase(name: string, args: string): string | null {
  const s = `${name} ${args}`.toLowerCase()
  if (/test|check|verify|validate|doctor|audit|smoke/.test(s)) return 'verify'
  if (/search|web_|source|paper|arxiv|doi|grep|glob|read|find|fetch|ls|cat/.test(s)) return 'research'
  if (/write|edit|str_replace|build|dev_build|dev_install|dev_inject|dev_reload|bash|run|npm|git|python|node|tsc/.test(s)) return 'build'
  if (/skill|distill|vault|teacher|expert|discussion|document|article|ppt|report|summary|copy|writing/.test(s)) return 'deliver'
  return null
}

function topTools(map: Map<string, number>, max = 3): string[] {
  return [...map.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, max)
...
function extractChangedFiles(nodes: any[]): string[] {
  const files = new Set<string>()
  const re = /"(?:file_path|path|file|files|target|outputPath|out)"\s*:\s*"([^"]+)"/g
  for (const n of nodes) {
    if (n.kind !== 'tool-result') continue
    const raw = n.call?.argsRaw ?? ''
    let m: RegExpExecArray | null
    while ((m = re.exec(raw)) !== null) {
      const p = m[1]
      if (p && !p.startsWith('node:') && !p.startsWith('http')) files.add(p)
    }
  }
  return [...files].slice(0, 12)
}

function traceLine(n: any): string | null {
  if (n.kind === 'tool-result') {
    const name = n.call?.name ?? 'tool'
    const content = truncateText(nodeText(n), 40)
    if (n.isError) return `工具 ${name} 出错${content ? `：${content}` : ''}`
  ...
  if (n.kind === 'turn-error') return `错误：${truncateText(n.message || '', 40)}`
  if (n.kind === 'turn-max-tokens') return `输出截断`
  if (n.kind === 'compaction') return `上下文压缩`
  return null
}
...
function buildWorkSummary(trajectory: any, status: any): WorkSummary {
  ...
  for (const n of nodes) {
    if (n.kind !== 'tool-result' && n.kind !== 'command') continue
  ...
  for (const r of trajectory?.runningCalls ?? []) {
    ...
    if (phase && phaseInfo[phase]) activePhase = phase
  }
  ...
  const recent = nodes.slice(-5).map((n: any) => {
    if (n.kind === 'user') return '收到目标'
    if (n.kind === 'assistant') return nodeText(n) ? '总结 / 回复' : '思考'
    if (n.kind === 'tool-result') return `工具：${n.call?.name ?? 'tool'}`
    if (n.kind === 'command') return `命令：${n.name ?? 'command'}`
    if (n.kind === 'turn-error' || n.kind === 'turn-max-tokens') return '错误'
    if (n.kind === 'compaction') return '压缩上下文'
    return n.kind
  })
  ...
  return {
    ...
    totalTools: nodes.filter((n: any) => n.kind === 'tool-result').length,
  }
}

function buildNarrative(summary: WorkSummary): string {
  const parts: string[] = []
  if (summary.goal) parts.push(`你现在在做「${summary.goal}」`)
  else parts.push('当前动作还在刚开始')

  const done: string[] = []
  const has = (k: string) => summary.phases.some((p) => p.key === k)
  if (has('research')) done.push('先把相关资料和事实确认了一遍')
  if (has('build')) {
    const files = summary.changedFiles.slice(0, 3)
    if (files.length) done.push(`然后把改动落到了 ${files.join('、')} 这些地方`)
    else done.push('然后开始把方案落到实际文件或代码里')
  }
  if (has('verify')) done.push('接着做了验证，确认没有破坏已有内容')
  if (has('deliver')) done.push('最后把结果整理成了可读的产出')

  if (done.length) parts.push(`：${done.join('，')}`)
  else if (summary.running.length) parts.push('，正在推进当前步骤')

  if (summary.running.length) parts.push('，现在还在继续做')
  if (summary.lastSummary) parts.push(`。目前的阶段结论：${summary.lastSummary}`)
...
const WORK_MAPS: Record<string, { title: string; stages: WorkStage[] }> = {
  research: {
    title: '科研生命周期图',
    stages: [
...
const ACTIVE_STAGE_BY_MAP: Record<string, Record<string, string>> = {
...
const STAGE_PHASE: Record<string, Record<string, string>> = {
...
const STAGE_NARRATIVE: Record<string, Record<string, string>> = {
  generic: {
...
function workMapKey(status: any): string {
  if (status?.route === 'core') return 'distill'
  const scenario = status?.scenario
  if (scenario === 'research') return 'research'
  if (scenario === 'teaching') return 'teacher'
  if (scenario === 'writing') return 'writing'
  if (status?.route === 'dev') return 'dev'
  return 'generic'
}

function stageStates(mapKey: string, summary: WorkSummary): { key: string; label: string; status: 'done' | 'active' | 'pending' }[] {
  const phases = new Map(summary.phases.map((p) => [p.key, p]))
  const cnt = (k: string) => phases.get(k)?.count ?? 0
...
function ArchifyWorkMap({ summary, status, selectedKey, onSelect }: { summary: WorkSummary; status: any; selectedKey?: string | null; onSelect?: (key: string) => void }): any {
  ...
  const colorOf = (status: 'done' | 'active' | 'pending') => {
    if (status === 'active') return 'var(--dsw-alias-state-business-primary, #3866b3)'
    if (status === 'done') return 'var(--dsw-alias-state-success-primary, #16794b)'
...
function AiBlackboxFlow(props: any): any {
  ...
  const trajectory = useTrajectory ? useTrajectory((s: any) => s) : null
  const summary = useMemo(() => buildWorkSummary(trajectory, status), [trajectory, status])
  const currentLabel = status?.label || '等待工作流'
  const mapKey = workMapKey(status)
  const selectedStage = (WORK_MAPS[mapKey]?.stages ?? []).find((s) => s.key === detailStage)
  const selectedState = detailStage ? stageStates(mapKey, summary).find((s) => s.key === detailStage)?.status ?? null : null
  const selectedPhaseKey = detailStage ? (STAGE_PHASE[mapKey]?.[detailStage] ?? '') : ''
    ...
    createElement('div', { className: 'ai-flow-head' }, [
      ...
      createElement('div', { className: 'ai-flow-sub' }, [
        ...
        summary.suggested[0]
          ...
          : null,
        createElement('span', { className: 'ai-flow-hint' }, ['实时更新']),
      ]),
    ]),
    ArchifyWorkMap({ summary, status, selectedKey: detailStage, onSelect: (key: string) => setDetailStage(key === detailStage ? null : key) }),
...
export function apply(ctx: ClientContext): void {
  ...
  ), '@dsh-external/dsh-skill-router: AI blackbox flow panel')
}
