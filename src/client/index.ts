...
function ArchifyFullFlow({ summary, status }: { summary: WorkSummary; status: any }): any {
  const phases = summary.phases.filter((p) => p.key !== 'goal')
  const mainW = 300
  const mainH = 66
  const chipH = 26
  ...
  const regionHeight = (p: WorkPhase): number => {
  ...
  let y = margin
  const drawNode = (label: string, sub: string, color: string): number => {
    elements.push(createElement('rect', {
      key: `n-${label}`,
      x: margin,
      y,
